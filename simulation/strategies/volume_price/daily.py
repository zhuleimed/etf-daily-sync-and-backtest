...
from strategies.volume_price.momentum_signals import compute_volume_price_signals, rank_etfs_by_volume_price
...
def main():
    ...
    MIN_HISTORY = max(MOMENTUM_WINDOW, VOL_LONG_PERIOD)   # 信号所需最少历史行数（对应各自回测的窗口）
    ...
    state_mgr = StateManager(str(STATE_FILE_DIR), "volume_price")
    ...
    engine = DailySimEngine(state_mgr=state_mgr, broker=broker, config={"initial_capital": INITIAL_CAPITAL},
        signal_func=compute_volume_price_signals, rank_func=rank_etfs_by_volume_price,
