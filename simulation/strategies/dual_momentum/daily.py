...
from simulation.strategies.dual_momentum.config import *
from strategies.dual_momentum.momentum_signals import compute_dual_momentum_signals, rank_etfs_by_dual_momentum
...
def main():
    ...
    MIN_HISTORY = max(MOMENTUM_WINDOW, ABS_MA)   # 信号所需最少历史行数（对应各自回测的窗口）
    ...
    state_mgr = StateManager(str(STATE_FILE_DIR), "dual_momentum")
    ...
    engine = DailySimEngine(state_mgr=state_mgr, broker=broker, config={"initial_capital": INITIAL_CAPITAL},
        signal_func=compute_dual_momentum_signals, rank_func=rank_etfs_by_dual_momentum,
    ...
    append_simulation_log("dual_momentum", SNAME, report, ETF_POOL)
