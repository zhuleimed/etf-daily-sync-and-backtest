"""
RSI 趋势确认轮动策略 — 每日模拟盘运行入口

调用方式：
    python -m simulation.strategies.rsi_trend_rotation.daily
...
from simulation.strategies.rsi_trend_rotation.config import (
...
def compute_rsi_signals(
    ...
    """RSI评分信号（兼容DailySimEngine接口）。"""
    return compute_rsi_scores(etf_data, today_idx)
...
def build_report(report: dict) -> list[str]:
    ...
    if not has_signal:
        if action == "hold":
            ...
            lines.append(f"  >> 今日信号: 空仓观望（无RSI>50的走强势ETF）")
        else:
    ...
    if ranking:
        ...
        if rank_parts:
            lines.append(f"      RSI排名: {' > '.join(rank_parts)}")
...
def main():
    ...
    state_mgr = StateManager(str(STATE_FILE_DIR), "rsi_trend_rotation")
    ...
    engine = DailySimEngine(
        ...
        signal_func=compute_rsi_signals,
        rank_func=rank_etfs_by_rsi,
        etf_pool=ETF_POOL,
    ...
    # 记录模拟盘日志
    append_simulation_log("rsi_trend_rotation", STRATEGY_NAME, report, ETF_POOL)
