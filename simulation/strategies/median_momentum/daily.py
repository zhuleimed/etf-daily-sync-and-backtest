...
from simulation.strategies.median_momentum.config import *
from strategies.median_momentum.momentum_signals import compute_median_signals, rank_etfs_by_median
logger = logging.getLogger("median_momentum")
SNAME = "中位数#2"
...
def main():
    ...
    MIN_HISTORY = MOMENTUM_WINDOW   # 信号所需最少历史行数（对应各自回测的窗口）
    ...
    state_mgr = StateManager(str(STATE_FILE_DIR), "median_momentum")
    ...
    engine = DailySimEngine(state_mgr=state_mgr, broker=broker, config={"initial_capital": INITIAL_CAPITAL},
        signal_func=compute_median_signals, rank_func=rank_etfs_by_median,
    ...
    append_simulation_log("median_momentum", SNAME, report, ETF_POOL)
