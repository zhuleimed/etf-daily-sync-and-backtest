...
from simulation.strategies.bollinger_reversion.config import *
from strategies.bollinger_reversion.momentum_signals import compute_bollinger_signals, rank_etfs_by_bollinger
logger = logging.getLogger("bollinger_reversion")
SNAME = "布林带回归"
...
def main():
    ...
    MIN_HISTORY = BB_PERIOD   # 信号所需最少历史行数（对应各自回测的窗口）
    lookback = max(MIN_HISTORY * 3, 120)
    ...
    for sym, df in etf_data.items():
        ...
        if mask.any() and df.index[mask][0] >= MIN_HISTORY:
            today_idx = df.index[mask][0]; break
    if today_idx is None: push_daily_report(SNAME, [f"{today_str}数据不足"]); return
    state_mgr = StateManager(str(STATE_FILE_DIR), "bollinger_reversion")
    ...
    engine = DailySimEngine(state_mgr=state_mgr, broker=broker, config={"initial_capital": INITIAL_CAPITAL},
        signal_func=compute_bollinger_signals, rank_func=rank_etfs_by_bollinger,
    ...
    append_simulation_log("bollinger_reversion", SNAME, report, ETF_POOL)
