...
from simulation.strategies.tail_risk.config import *
from strategies.tail_risk.momentum_signals import compute_tail_risk_signals, rank_etfs_by_tail_risk
...
def main():
    ...
    MIN_HISTORY = max(VOL_WINDOW, 22)   # 信号所需最少历史行数（对应各自回测的窗口）
    ...
    state_mgr = StateManager(str(STATE_FILE_DIR), "tail_risk")
    ...
    engine = DailySimEngine(state_mgr=state_mgr, broker=broker, config={"initial_capital": INITIAL_CAPITAL},
        signal_func=compute_tail_risk_signals, rank_func=rank_etfs_by_tail_risk,
    ...
    append_simulation_log("tail_risk", SNAME, report, ETF_POOL)
