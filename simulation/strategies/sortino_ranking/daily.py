...
from simulation.strategies.sortino_ranking.config import *
from strategies.sortino_ranking.momentum_signals import compute_sortino_signals, rank_etfs_by_sortino
logger = logging.getLogger("sortino_ranking")
SNAME = "Sortino排名"
...
def main():
    ...
    MIN_HISTORY = max(MOMENTUM_WINDOW, VOL_WINDOW)   # 信号所需最少历史行数（对应各自回测的窗口）
    ...
    state_mgr = StateManager(str(STATE_FILE_DIR), "sortino_ranking")
    ...
    engine = DailySimEngine(state_mgr=state_mgr, broker=broker, config={"initial_capital": INITIAL_CAPITAL},
        signal_func=compute_sortino_signals, rank_func=rank_etfs_by_sortino,
    ...
    append_simulation_log("sortino_ranking", SNAME, report, ETF_POOL)
