...
from simulation.strategies.spread_reversion.config import *
from strategies.spread_reversion.momentum_signals import compute_spread_signals, rank_etfs_by_spread
logger = logging.getLogger("spread_reversion")
SNAME = "价差回归"
...
def main():
    ...
    MIN_HISTORY = LOOKBACK + 10   # 信号所需最少历史行数（对应各自回测的窗口）
    ...
    state_mgr = StateManager(str(STATE_FILE_DIR), "spread_reversion")
    ...
    engine = DailySimEngine(state_mgr=state_mgr, broker=broker, config={"initial_capital": INITIAL_CAPITAL},
        signal_func=compute_spread_signals, rank_func=rank_etfs_by_spread,
    ...
    append_simulation_log("spread_reversion", SNAME, report, ETF_POOL)
