"""Sortino排名引擎: 按收益/下行波动率排名,选稳涨而非猛涨"""
...
class SortinoRankingEngine(BacktestEngine):
    ...
    def _score(self, sym, idx):
        ...
        neg=rets[rets<0]; down_vol=neg.std()*np.sqrt(252) if len(neg)>1 else rets.std()*np.sqrt(252)
        return ret/max(down_vol,0.01)
