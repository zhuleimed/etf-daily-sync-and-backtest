...
class BacktestEngine:
    ...
    def run(self) -> "BacktestEngine":
        ...
        for idx in range(n):
            ...
            # ── Step 4: 动量信号（支持动态窗口 + 相对动量）──
            if self.dynamic_window:
                momentum = compute_momentum_signals_dynamic(
                    self.etf_data, idx, threshold=WINDOW_SWITCH_THRESHOLD,
                )
            else:
                momentum = compute_momentum_signals(self.etf_data, idx, self.momentum_window)
            ...
            if USE_RELATIVE_MOMENTUM and self.etf_benchmark_data:
                for sym in momentum.index:
                    if sym in ETF_BENCHMARK_MAP:
                        ...
                        if idx_df is not None and idx < len(idx_df):
                            idx_mom = idx_df.iloc[idx].get("momentum", np.nan)
    ...
    def _make_decision_single(self, idx: int, today_data: Dict,
        ...
        # 短期动量确认：目标ETF的5日动量不能为负
        if SHORT_TERM_MOMENTUM_CHECK and idx >= 5:
