...
>
> 📖 非技术读者请看大白话版：[`../docs/REWIRE_EXPLAINED_20260925.md`](../docs/REWIRE_EXPLAINED_20260925.md)
...
3. **`tail_risk` 回测端的分支从未触发过**（已于 2026-09-25 一并修复，见下）。

### `tail_risk` 回测死分支修复（2026-09-25）

`TailRiskEngine.__init__` 原把 `hs300_data` 硬编码为 `None`，`_check_tail` 恒返回 False，
**"尾部风险"分支在回测里从未触发过**，策略退化成纯动量。
修复方式：不新增加载逻辑——基类 `load_data` **本来就**加载了沪深300基准
（`self.benchmark_data`），只要在 `load_data` 末尾把它接到 `hs300_data` 即可；
同时允许 `hs300_data` 作为 kwarg 显式注入。

修复前后对比（同参数、同数据，`_check_tail` 触发天数从 0 变为 39）：

| 期间 | 修复前累计 | 修复后累计 | 修复前回撤 | 修复后回撤 | 尾部触发 | 尾部进场 |
|---|---|---|---|---|---|---|
| 2024 全年 | 19.06% | 7.52% | −25.31% | −27.73% | 13 | 6 |
| 2025 全年 | 39.29% | 33.26% | −18.07% | −18.07% | 6 | 2 |
| 2026 至今 | 1.64% | **9.01%** | −21.63% | **−17.61%** | 15 | 3 |
| 全周期 | 86.15% | 70.25% | −25.49% | −34.10% | 39 | 12 |

**怎么读这张表**（重要，别只看全周期）：

- 机制**确实在工作**了：39 个触发日、12 次尾部进场（修复前只有首次建仓那 1 次）。
- **2026 年（有急跌的年份）它是帮忙的**：收益 +7.4pp、回撤改善 4pp —— 这正是它被设计出来的场景。
- **2024/2025（平静年份）它是拖累的**：合计 −17.5pp。典型的"保险要交保费"。
- 全周期 −15.9pp 意味着**这条历史路径上保费超过了赔付**。但这是**单条路径**的证据，
  不足以否定机制本身——是否保留该分支属策略取舍，不是 bug 问题。**本次只修正确性，不改设计。**

> 一致性收益：修复后，**模拟盘与回测在 tail_risk 上重新自洽**了。
> 在此之前是"实盘无尾部逻辑（旧 daily.py 用动量信号）+ 回测也无（死分支）"——
> 两边都错但碰巧一致；接线修复后一度变成"实盘有、回测无"，本次修复补齐。

#### 该不该留这个机制？——两个改进假设都被实测否定

修好之后自然会问："它亏在哪儿，能不能改好？" **先量，别猜**：

| 测量 | 结果 |
|---|---|
| 663 个交易日触发 | 39 天（5.9%），集中在 2026-06~07 急跌期 |
| 避险持仓总天数 | 119 天（≈18%） |
| 其中「警报仍在」 | 28 天 |
| 其中「**警报已解除但仍未归位**」 | **91 天（76%）** |

第 4 行看起来像个大漏洞 → 提出假设一「警报解除即归位」。
另外 2024-02 曾出现 3 天换 3 次 → 提出假设二「入场冷静期」。
两个都实现了开关（`tail_immediate_exit` / `tail_entry_cooldown`，默认关）并跑 4 期对比：

| 变体 | 2024 全年 | 2025 全年 | 2026 至今 | 全周期 |
|---|---|---|---|---|
| **现状（默认）** | 7.52% / −27.73% | 33.26% / −18.07% | **9.01% / −17.61%** | **70.25% / −34.10%** |
| 假设一：立即归位 | 7.05% / −29.32% | 33.26% / −18.07% | −11.48% / −30.57% | 30.16% / −38.85% |
| 假设二：入场冷静期 3 天 | 7.32% / −27.76% | 33.26% / −18.07% | 9.01% / −17.61% | 70.10% / −34.10% |
| 假设二：5 天 | 7.32% / −27.76% | 33.26% / −18.07% | 9.01% / −17.61% | 70.10% / −34.10% |
| 假设二：10 天 | 7.32% / −27.76% | 33.26% / −18.07% | 9.01% / −17.61% | 70.10% / −34.10% |

（格式：累计收益率 / 最大回撤）

**结论 —— 两个假设都不成立，维持现状：**

1. **「警报解除即归位」严重变差**：2026 从 +9.01% 打成 −11.48%，回撤从 −17.6% 恶化到 −30.6%；
   全周期 70.25%→30.16%。**"慢归位"其实有保护作用**——警报解除只说明"跌速放缓"，
   不等于危险过去；急着回到动量榜最强的那只，往往正是刚被打回原形的仓位。
2. **「入场冷静期」三档结果完全相同**，说明除 2024-02 那一次外，
   **根本没发生过"几天内换另一只避险ETF"** —— churn 不是成本主因。
   而且它让 2024 年**略微变差**（−0.2pp），说明那次换来换去的结果并不坏。

**因此：这个机制的成本是固有的，不是可修的实现缺陷。**
它就是一个真实的取舍——平稳年交保费，急跌年拿赔付。**是否保留属风险偏好决策，不是 bug 问题。**

两个开关作为"已否定的假设"保留在代码里（默认关闭，注释写明结论），
目的是**防止以后有人重复提出同一方案、又重写一遍**。
