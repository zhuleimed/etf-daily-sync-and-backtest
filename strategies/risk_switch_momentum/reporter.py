...
职责：
  1. 生成回测结果 CSV（净值日报表、交易明细表、绩效汇总表）
  2. 绘制净值对比图、回撤曲线图、持仓热力图、月度收益热力图
  3. 控制台输出绩效摘要
...
class Reporter:
    ...
    def plot_equity_curve(
        ...
        # 策略净值
        ax.plot(dates, strategy_net, label="策略净值",
        ...
        ax.set_title("宽基ETF动量轮动策略 - 净值曲线", fontsize=14, fontweight="bold")
        ax.set_xlabel("交易日期")
        ax.set_ylabel("净值")
    ...
    def plot_holding_heatmap(self, daily_df: pd.DataFrame,
        ...
        im = ax.imshow(codes, aspect="auto", cmap=cmap, norm=norm,
                       interpolation="nearest")

        # 色标
        cbar = plt.colorbar(im, ax=ax, ticks=list(range(len(symbols) + 1)),
                            shrink=0.6)
    ...
    def plot_monthly_returns(self, daily_df: pd.DataFrame,
        ...
        # 计算每日收益率，按年月聚合
        # 月度收益 = 当月最后一日的累计收益 / 前一月最后一日的累计收益 - 1
        monthly_ret = df.groupby(["year", "month"]).last()[
            "cumulative_return"
        ].pct_change().fillna(0)
        ...
        cmap = plt.cm.RdYlGn
        im = ax.imshow(pivot_monthly.values * 100, aspect="auto",
