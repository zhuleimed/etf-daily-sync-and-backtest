"""Sharpe 排名 — 模拟盘信号函数（自 strategies/sharpe_ranking/engine.py 移植）

原回测逻辑（SharpeRankingEngine._score）：
    收益 = 收盘/前 MOMENTUM_WINDOW 日收盘 − 1
    波动 = 近 VOL_WINDOW 日收益率标准差 × √252（年化）
    得分 = 收益 / max(波动, 0.01)

    与"纯动量"的区别：同样涨 5%，波动小的得分更高——即"性价比"而非"涨幅"。

移植说明：
    得分函数逐行照搬原回测，未做改动。引擎侧只取排名第 1，与原回测一致。
    得分可能为负（下跌的 ETF），引擎只在目标得分 > 0 时才开仓，与原回测
    _make_decision_single 的 `if target_mom > 0` 判断一致。
"""

from __future__ import annotations

import numpy as np
import pandas as pd

from . import config as cfg


def compute_sharpe_signals(
    etf_data: dict[str, pd.DataFrame],
    date_idx: int,
    momentum_window: int = 20,
) -> pd.Series:
    """计算各 ETF 的 Sharpe 得分（收益 ÷ 年化总波动率）。"""
    window = cfg.MOMENTUM_WINDOW
    vol_window = cfg.VOL_WINDOW

    scores: dict[str, float] = {}
    for sym in cfg.ETF_SYMBOLS:
        df = etf_data.get(sym)
        if df is None or date_idx < max(window, vol_window) or date_idx >= len(df):
            scores[sym] = np.nan
            continue

        ret = df["close"].iloc[date_idx] / df["close"].iloc[date_idx - window] - 1
        rets = df["pct_chg"].iloc[date_idx - vol_window + 1: date_idx + 1]
        vol = rets.std() * np.sqrt(252) if len(rets) > 1 else 0.5
        scores[sym] = ret / max(vol, 0.01)

    return pd.Series(scores, dtype=float)


def rank_etfs_by_sharpe(scores: pd.Series) -> pd.Series:
    """按得分降序排列，返回 {1: 最优ETF代码, 2: 次优, ...}。"""
    valid = scores.dropna()
    if valid.empty:
        return pd.Series(dtype=str)
    return pd.Series(
        valid.sort_values(ascending=False).index.values,
        index=range(1, len(valid) + 1),
    )
