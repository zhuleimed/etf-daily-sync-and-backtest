"""双动量引擎: price>SMA(绝对趋势) + 动量排名(相对强度)"""
import numpy as np
...
class DualMomentumEngine(BacktestEngine):
    def __init__(self, **kw):
        ...
        self.abs_ma = kw.get('abs_ma', cfg.ABS_MA)
    
    def _above_ma(self, sym, idx):
        if idx < self.abs_ma: return True
        df = self.etf_data[sym]; close=df.iloc[idx]["close"]
        return close > df.iloc[idx-self.abs_ma+1:idx+1]["close"].mean()
    ...
    def run(self):
        ...
        for idx in range(n):
            ...
            for sym in mom.index:
                if not self._above_ma(sym,si) and not np.isnan(mom[sym]): mom[sym]=np.nan
            ranking=rank_etfs_by_momentum(mom); target=ranking.get(1) if len(ranking)>0 else None
