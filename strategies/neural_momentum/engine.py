...
class BacktestEngine:
    ...
    def __init__(self, initial_capital: float = INITIAL_CAPITAL,
                 ...
                 neural_scores: Optional[pd.DataFrame] = None,
        ...
        # ── 风险状态择时开关（默认关闭：threshold=1.0 永不触发） ──
        # risk_scores: {date_str: 风险分}, risk_threshold: 风险分>阈值 → 空仓避险
        self.risk_scores: Dict[str, float] = risk_scores or {}
        self.risk_threshold: float = risk_threshold
        self.risk_off_flag: bool = False   # 供决策使用（T-1 风险状态）
        # ── Neural Momentum 混合信号 ──
        # neural_scores: DataFrame(date × ETF 预测分，已 z-score)
        # weight_w: score = w × 动量z + (1-w) × 神经z
        self.neural_scores: Optional[pd.DataFrame] = neural_scores
        self.weight_w: float = weight_w
        # 最小持仓天数追踪
    ...
    def run(self) -> "BacktestEngine":
        ...
        for idx in range(n):
            ...
            # ── Neural Momentum 混合信号：w × 动量z + (1-w) × 神经z ──
            if self.neural_scores is not None and self.weight_w < 1.0:
                m_valid = momentum.dropna()
                if len(m_valid) >= 3:
                    m_z = (m_valid - m_valid.mean()) / (m_valid.std() + 1e-9)
                    date_str = self.dates[signal_idx].strftime("%Y-%m-%d")
                    n_row = (self.neural_scores.loc[date_str]
                             if date_str in self.neural_scores.index else None)
                    merged = {}
                    for sym in momentum.index:
                        ...
                        if n_row is not None and sym in n_row.index and not pd.isna(n_row[sym]):
                            nz = float(n_row[sym])
                        merged[sym] = self.weight_w * float(mz) + (1 - self.weight_w) * nz
                    if merged:
                        momentum = pd.Series(merged)
            ...
            # ── Step 4.5: 风险状态择时开关（对齐信号时点 idx-1） ──
            # 风险分用 T-1 日（与动量信号同点），T 日执行
            self.risk_off_flag = False
            if self.risk_threshold < 1.0:
                risk_date = self.dates[signal_idx].strftime("%Y-%m-%d")
                risk_score = self.risk_scores.get(risk_date, 0.0)
                self.risk_off_flag = risk_score > self.risk_threshold
                if self.risk_off_flag and has_position and hold_symbol:
                    # 风险触发：清仓避险（T 日开盘价执行）
                    self._execute_risk_exit(idx, today_data, "sell", "风险状态择时触发")
                    self._record_day(idx, today_data, action_override="risk_off")
    ...
    def _make_decision_single(self, idx: int, today_data: Dict,
        ...
        if not has_position:
            if target_mom > 0 and not self.risk_off_flag:
