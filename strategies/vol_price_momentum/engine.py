    ...
    def run(self) -> "BacktestEngine":
        ...
        for idx in range(n):
            ...
            for sym in ETF_SYMBOLS:
                ...
                if df is None or signal_idx < 20:
                    rankings[sym] = -999
                else:
                    mom = (df.iloc[signal_idx]["close"] / df.iloc[signal_idx - 20]["close"] - 1)
                    vol_ma = df.iloc[signal_idx - 20: signal_idx + 1]["volume"].mean()
                    vol_ratio = df.iloc[signal_idx]["volume"] / max(vol_ma, 1)
                    rankings[sym] = mom * vol_ratio
