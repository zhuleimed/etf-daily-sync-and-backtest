...
与其它策略一致的风控体系（均值回归使用A模式，保留以保持架构一致）。
"""
...
def run_all_risk_checks(risk_state, total_value, has_position, hold_symbol, current_high, current_low, current_close, current_atr, mode="B"):
    if mode == "A":
        return "none", ""
    triggered, reason = _check_extreme_drawdown(risk_state, total_value)
    if triggered:
        return "extreme_drawdown", reason
    if mode == "B" and has_position and hold_symbol:
        triggered, reason = _check_stop_loss(risk_state.entry_price, current_low, current_atr)
        if triggered:
            return "stop_loss", reason
        floating_return = (current_close - risk_state.entry_price) / risk_state.entry_price
        triggered, reason = _check_stop_profit(risk_state, current_low, floating_return)
        if triggered:
            return "stop_profit", reason
    return "none", ""


def _check_stop_profit(risk_state, current_low, floating_return):
...
def _check_stop_loss(entry_price, current_low, current_atr):
...
def _check_extreme_drawdown(risk_state, current_total_value):
    if risk_state.peak_total_value <= 0:
        return False, ""
    drawdown_ratio = (risk_state.peak_total_value - current_total_value) / risk_state.peak_total_value
