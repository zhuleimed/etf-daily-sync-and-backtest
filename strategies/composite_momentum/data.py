...
职责：
  1. 从 SQLite 加载 ETF 日线数据（含多窗口动量预计算）
  2. 日期对齐
...
from .config import ETF_SYMBOLS, DB_PATH, TREND_WINDOWS
...
def load_all_etf_data(
    ...
    预计算多个动量窗口和辅助列，供多因子打分使用。
    """
    if symbols is None:
        symbols = ETF_SYMBOLS

    # 动量窗口数据前移
    max_window = max(TREND_WINDOWS.values())
    start_dt = pd.to_datetime(start_date)
    extended_start = (start_dt - timedelta(days=max_window * 3 + 60)).strftime("%Y-%m-%d")

    etf_data: dict[str, pd.DataFrame] = {}
    for sym in symbols:
        df = _load_single_etf(sym, extended_start, end_date, db_path)
        if df is not None and len(df) > max_window:
            etf_data[sym] = df
...
def _load_single_etf(
    ...
    # 多窗口动量（供趋势因子使用）
    df["momentum"] = df["close"] / df["close"].shift(max(TREND_WINDOWS.values())) - 1
    for name, window in TREND_WINDOWS.items():
        df[f"momentum_{name}"] = df["close"] / df["close"].shift(window) - 1
...
def load_index_data(
    ...
    max_window = max(TREND_WINDOWS.values())
    start_dt = pd.to_datetime(start_date)
    extended_start = (start_dt - timedelta(days=max_window * 3)).strftime("%Y-%m-%d")
