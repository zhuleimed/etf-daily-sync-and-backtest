...
与 ADX/composite_momentum 引擎结构一致，但决策使用 RSI 评分：
  1. 数据加载
  2. 风控检查 → 若触发则平仓
  3. 计算 RSI + RSI斜率 + 短期动量 → 综合评分 → 排名
  4. 决策：RSI>50 的标的才能买入；RSI≤50 → 空仓
  5. 记录
"""
...
class BacktestEngine:
    ...
    def run(self) -> "BacktestEngine":
        ...
        print(f"  RSI周期: {14}, 多头阈值: {RSI_BULL_THRESHOLD}")
        print(f"  权重: RSI值{50}% + RSI斜率{25}% + 短期动量{25}%")
        ...
        for idx in range(n):
            ...
            # ── RSI 评分（用 idx-1 避免 look-ahead bias） ──
            signal_idx = max(0, idx - 1)
            index_aligned = None
            if not self.index_data.empty and signal_idx < len(self.index_data):
                index_aligned = self.index_data

            scores = compute_rsi_scores(self.etf_data, signal_idx)
            ranking = rank_etfs_by_rsi(scores)
            spread = compute_rsi_spread(scores)
    ...
    def _make_decision(self, idx: int, today_data: dict, hold_symbol: Optional[str],
        ...
        """
        RSI 决策逻辑：
          1. 无持仓 → 若目标得分>0（RSI>50且截面得分最高），开仓
          2. 熊市 → 额外保守，得分>0.5才开仓
          3. 有持仓 → 若目标得分 > 当前得分 + 阈值，切换
          4. 若当前持仓得分变为0（RSI跌破50或所有ETF均弱势）→ 平仓
        """
        ...
        if has_position and not pd.isna(current_score) and current_score <= 0:
            self._sell(hold_symbol, idx, today_data, reason="RSI跌破50/信号消失，平仓")
            has_position = False
            hold_symbol = None
        ...
        if not has_position:
            if target_etf is not None and not pd.isna(target_score) and target_score > 0:
                ...
                self._buy(target_etf, self.cash * 0.98, idx, today_data,
                         reason=f"开仓 {target_etf}（RSI评分={target_score:.4f}）")
            ...
            if score_diff > min_diff:
                if ADJUSTMENT_DAYS > 1 and hold_symbol:
                    ...
                    self._sell(hold_symbol, idx, today_data,
                              reason=f"切换 {hold_symbol}→{target_etf}（RSI分差={score_diff:.4f}）")
                    self._buy(target_etf, self.cash * 0.98, idx, today_data,
                             reason=f"买入 {target_etf}（RSI评分={target_score:.4f}）")
                    self.risk_state.on_open_position(today_data[target_etf]["open"])
                    self._days_since_last_switch = 0

    def _record_day(self, idx: int, today_data: dict, action_override: str = "",
                    regime: str = "neutral", top_etf: str = "", rsi_score: float = 0.0):
        total_value = self._calc_total_value(today_data)
        ...
        self.daily_records.append(DailyRecord(
            ...
            regime=regime, top_etf=top_etf, rsi_score=rsi_score,
        ))
    ...
    def get_daily_df(self) -> pd.DataFrame:
        ...
        records = [{
            ...
            "regime": r.regime, "top_etf": r.top_etf, "rsi_score": r.rsi_score,
