...
STRATEGY_DISPLAY_NAME = "RSI趋势确认轮动"
...
class Reporter:
    ...
    def plot_equity_curve(
        ...
        ax.plot(dates, strategy_net, label=STRATEGY_DISPLAY_NAME,
    ...
    def plot_drawdown(self, daily_df: pd.DataFrame,
        ...
        ax.set_title(f"{STRATEGY_DISPLAY_NAME} - 回撤曲线", fontsize=13, fontweight="bold")
        ax.set_xlabel("交易日期")
        ax.set_ylabel("回撤 (%)")
    ...
    def plot_holding_heatmap(self, daily_df: pd.DataFrame,
        ...
        ax.set_yticks([])
        ax.set_title(f"{STRATEGY_DISPLAY_NAME} - 持仓分布热力图（月度末）", fontsize=13, fontweight="bold")
        plt.tight_layout()
        path = os.path.join(self.output_dir, filename)
        fig.savefig(path, dpi=150, bbox_inches="tight")
        plt.close(fig)
    ...
    def plot_monthly_returns(self, daily_df: pd.DataFrame,
        ...
        ax.set_title(f"{STRATEGY_DISPLAY_NAME} - 月度收益率热力图 (%)", fontsize=13, fontweight="bold")
