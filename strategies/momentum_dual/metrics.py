...
class BacktestMetrics:
    ...
    benchmark_return: float = 0.0          # 沪深300
    ew_benchmark_return: float = 0.0       # 等权组合
    ...
    total_round_trips: int = 0             # 完整买卖轮次
    switch_count: int = 0                  # 调仓切换次数
    avg_hold_days: float = 0.0
    win_rate: float = 0.0                  # 按日胜率
    trade_win_rate: float = 0.0            # 交易胜率
    profit_factor: float = 0.0             # 盈亏比
    total_trade_cost: float = 0.0
    cost_ratio: float = 0.0                # 交易成本占比
...
class MetricsCalculator:
    ...
    def compute(
        ...
        ew_benchmark_return: Optional[float] = None,
    ) -> BacktestMetrics:
        """计算完整绩效指标。"""
        ...
        metrics.annualized_return = (
            (1 + total_return) ** (TRADING_DAYS_PER_YEAR / n) - 1
        )
        ...
        metrics.downside_volatility = (
            downside.std() * np.sqrt(TRADING_DAYS_PER_YEAR)
        ...
        if std_daily > 0:
            metrics.sharpe_ratio = (
                (avg_daily - rf_daily) / std_daily * np.sqrt(TRADING_DAYS_PER_YEAR)
            )
        if downside_std > 0:
            metrics.sortino_ratio = (
                (avg_daily - rf_daily) / downside_std * np.sqrt(TRADING_DAYS_PER_YEAR)
            )
        ...
        metrics.total_trade_cost = sum(
            t.commission + t.tax for t in trade_records
        )
        metrics.cost_ratio = (
            metrics.total_trade_cost / initial_capital
            if initial_capital > 0 else 0.0
        )

        # 调仓切换次数（每个完整买卖轮视为一次实际切换）
        # 找出所有 "调仓买入" 记录，按日期分组去重，每组为一次切换
        switch_trades = set()
        for t in trade_records:
            if t.trade_type in ("买入",):
                switch_trades.add(t.date)
            elif "调仓买入" in t.trade_type:
                # 渐进调仓的第一次（"第1天"）对应一次新切换
                if "第1天" in t.reason:
                    switch_trades.add(t.date)
        metrics.switch_count = len(switch_trades)

        # 平均持仓天数（从卖出/虚拟卖出记录中获取）
        ...
        profit_trades = [t for t in trade_records if t.trade_type in
                         ("卖出", "止损卖出", "止盈卖出", "调仓卖出",
                          "风控卖出", "极端回撤清仓", "虚拟卖出")]
        # 从卖出交易计算盈亏（需要穿越回买入成本，engine里应该记录）
        if profit_trades:
