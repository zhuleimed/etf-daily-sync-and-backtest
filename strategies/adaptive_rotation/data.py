...
import numpy as np
import pandas as pd
from .config import ETF_SYMBOLS, DB_PATH, REGIME_MA_PERIOD, MOM_WINDOW


def load_all_etf_data(symbols=None, start_date="2024-01-01", end_date="", db_path=DB_PATH):
    if symbols is None:
        symbols = ETF_SYMBOLS
    buffer = max(REGIME_MA_PERIOD, MOM_WINDOW) + 30
...
def _load_single_etf(symbol, start_date, end_date, db_path=DB_PATH):
    with sqlite3.connect(db_path) as conn:
        query = "SELECT date, open, high, low, close, volume FROM etf_daily WHERE symbol = ? AND date >= ?"
...
def load_index_data(symbol="000300", start_date="2024-01-01", end_date="", db_path=DB_PATH):
    buffer = max(REGIME_MA_PERIOD, MOM_WINDOW) + 30
    start_dt = pd.to_datetime(start_date)
    extended_start = (start_dt - timedelta(days=buffer * 3)).strftime("%Y-%m-%d")
    with sqlite3.connect(db_path) as conn:
        query = "SELECT date, close FROM index_daily WHERE symbol = ? AND date >= ?"
        params = [symbol, extended_start]
        if end_date:
            query += " AND date <= ?"
            params.append(end_date)
        query += " ORDER BY date"
