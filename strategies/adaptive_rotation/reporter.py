...
class Reporter:
    ...
    def plot_equity_curve(self, daily_df, benchmark_data=None, ew_data=None, filename="equity_curve.png"):
        ...
        ax.plot(dates, strategy_net, label=STRATEGY_DISPLAY_NAME, color="#8E44AD", linewidth=2, zorder=5)
        if benchmark_data is not None and not benchmark_data.empty:
            ax.plot(dates, benchmark_data["cumulative_returns"].values[:len(strategy_net)],
                    label="沪深300", color="#A23B72", linewidth=1.5, linestyle="--", alpha=0.8)
        if ew_data is not None and not ew_data.empty:
            ax.plot(dates, ew_data["cumulative_returns"].values[:len(strategy_net)],
                    label="等权组合", color="#F18F01", linewidth=1.5, linestyle="-.", alpha=0.8)
    ...
    def print_summary(self, metrics):
        ...
        print(sep)
        for name, keys in [("收益指标", ["累计收益率","年化收益率","沪深300收益","超额(沪深300)"]),
                          ("风险指标", ["最大回撤","年化波动率"]),
                          ("风险调整收益", ["夏普比率"]),
                          ("交易统计", ["调仓切换次数","日胜率"]),
                          ("资金信息", ["初始资金","最终资金"])]:
