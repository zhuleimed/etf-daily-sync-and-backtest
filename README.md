...
| **21** | **资产配置(风险平价)** 🆕🏦 | `asset_allocation/` | 6类低相关资产+月度再平衡（2023-2025夏普2.0+，最大年回撤-4.2%） | **配置分散** |
...
| # | 推送 | 策略数 | 方式 |
|---|------|--------|------|
| 1 | 📊 动量类策略合集 | 9合1 | batch（`BATCH_MODE=1`） |
| 2 | 📊 候选策略合集 | 8合1 | batch（`BATCH_MODE=candidate`） |
| 3 | 🥇 黄金避险轮动 | 独立 | 直推 |
| 4 | 🌏 跨境轮动 | 独立 | 直推 |
| 5 | 📊 ETF模拟盘汇总 | 全策略 | summary.py |

所有日报统一 T+1 三板块格式（2026-07-24 重构，见 `report_builder.py`）：
```
策略名 | 日期
【昨信号执行】              ← T日信号 → T+1日开盘执行
【今日新信号（明日开盘执行）】  ← T+1日收盘计算 → T+2日执行
【账户日结】                ← 持仓、市值、现金、收益率
```

> 历史：原 12 条独立推送 → 2026-07-22 合并为 5 条（含汇总），2026-07-24 新增候选批次。
