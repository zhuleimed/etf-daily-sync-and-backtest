...
python -m strategies.market_breadth.run            # 跑市场宽度择时回测 🆕🔥
python -m strategies.composite_momentum.run          # 跑复合动量回测
...
python -m simulation.strategies.market_breadth.daily   # 手动跑市场宽度择时 🆕🔥
...
| **21** | **市场宽度择时** 🆕🔥 | `market_breadth/` | 宽度>70%动量+宽度<30%空仓 | **择时+动量** |
...
**㉑ market_breadth — 市场宽度择时（+285%, 夏普1.88）🆕🔥 全策略收益第一**
```
市场宽度(站上MA20的ETF占比)择时：
  宽度 > 70% → 强市，正常20日动量轮动
  宽度 < 30% → 熊市，全部空仓避险
  宽度 30-70% → 中性，空仓等待方向
```
参数: MA=20, STRONG=70%, WEAK=30%, NEUTRAL=cash
特点: 解决"什么时候不该在场"的根本问题。2024+61%、2025+63%(夏普2.38)、2026YTD+41%(HS300仅+0.5%)。
全策略收益第一(+285%)，仅155笔交易。核心优势：在急跌时自动空仓，反弹时才进场。
...
|---|------|--------|------|---------|------|------|
| **1** | **market_breadth 市场宽度择时** 🆕🔥 | **+285%** | **1.88** | -26% | 74% | 156 |
| **2** | **composite_momentum 复合动量** 🔥 | **+237%** | **1.77** | -17% | 67% | 53 |
| **3** | **cross_border 跨境轮动** 🆕🏆 | **+139%** | **1.73** | **-20%** | **43%** | **315** |
| **3** | **adaptive_rotation 自适应轮动** 🆕 | **+126%** | **1.76** | **-15%** | **43%** | **25** |
| 4 | ma_etf 逐ETF均线 | +191% | 1.50 | -26% | 55% | — |
| 5 | ma_filter MA=250 | +167% | 1.37 | -26% | 51% | — |
| 6 | rotation 纯动量 | +133% | 1.19 | -26% | 44% | 19 |
| 7 | **vol_filter 波动率过滤** | **+117%** | **1.31** | **-23%** | **41%** | **19** |
